   ...
   // ==========================================
    ...
    public ResponseEntity<Map<String, Object>> register(@RequestBody Map<String, String> request) {
        ...
        if ("pencari".equalsIgnoreCase(role)) {
            ...
            penyewa.setNamaPenyewa(name); 
            penyewa.setEmail(email);
            penyewa.setRole("pencari");
            akunBaru = penyewa;
        } else {
            // Jika dia mendaftar sebagai pemilik
            Penyewa pemilikBaru = new Penyewa(); 
            pemilikBaru.setUsername(username);
            pemilikBaru.setPassword(password);
            pemilikBaru.setNamaLengkap(name);
            pemilikBaru.setNamaPenyewa(name); // Memastikan nama penyewa tidak kosong
            pemilikBaru.setEmail(email);
            pemilikBaru.setRole("pemilik");
            akunBaru = pemilikBaru;
        }

        // KUNCI PERBAIKAN: Tangkap kembali objeknya agar ID dari database terbaca dan tidak Null
        akunBaru = akunRepository.save(akunBaru);
        ...
        Map<String, Object> userData = new HashMap<>();
        userData.put("id", akunBaru.getId().toString()); // Sekarang aman, ID tidak akan Null
